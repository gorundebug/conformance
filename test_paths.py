...
class DependencyRootTest(unittest.TestCase):
    ...
    def test_cold_gate_runner_is_ordered_fails_fast_and_resumable(self) -> None:
        ...
        self.assertIn('if [ "$resume" -eq 0 ]; then\n  cleanup_log=', runner)
        self.assertEqual(runner.count("docker builder prune --all --force"), 1)
        self.assertIn("clearing all BuildKit cache once before the profile", runner)
        ...
        self.assertIn('"$gate" "$status" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$log_file"', runner)
