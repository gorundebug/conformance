...
Profiling is part of the conformance repository. Its quickstart prepares every
managed dependency and runs the profiler with sensible defaults:

```bash
git clone https://github.com/gorundebug/conformance.git
cd conformance
bash ./quickstart.sh -- profiling-all
```
...
```bash
bash ./quickstart.sh --profile current -- profiling-all
...
`main` branch advances. Profiling and benchmarks share these pinned native
copies under `.dependencies/performance-native`; conformance's ordinary native
scenario checkouts remain independent on `main`.
...
Use `bash ./quickstart.sh --clone-only` to just fetch the sibling repos without
running anything.
...
```bash
python3 profiling/examples/run.py --language typescript --language typescript-native \
  --scenario timeout
python3 profiling/examples/run.py --language typescript --language typescript-native \
...
```bash
python3 profiling/examples/run.py --skip-build   # reuse already-built images
python3 profiling/examples/run.py --clean        # tear down and remove artifacts
```
