...
class TypeScriptProfilingTest(unittest.TestCase):
    ...
    def test_language_matrix_and_environment(self) -> None:
        ...
        self.assertTrue(
            all(item.example.parent == profiling.NATIVE_ROOT for item in native)
        )
        self.assertTrue(
            all(
                item.example.parent == profiling.ROOT
                for item in profiling.LANGUAGES
    ...
    def test_cpp_normal_profile_uses_kafka_free_override(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("orderEventsSaslMechanism:", variables)
            self.assertIn("orderEventsSecurityProtocol:", variables)
            self.assertEqual(
                override,
