...
NATIVE_ROOT = Path(
    os.environ.get(
        ...
        str(PROFILING_ROOT.parent / ".dependencies" / "performance-native"),
    )
).expanduser().resolve()
...
LANGUAGES = (
    ...
    Language(
        "go-native", NATIVE_ROOT / "gonativeexample",
    ...
    Language(
        "cpp-native", NATIVE_ROOT / "cppnativeexample",
    ...
    Language(
        "cppboost-native", NATIVE_ROOT / "cppboostnativeexample",
    ...
    Language(
        "python-native", NATIVE_ROOT / "pynativeexample",
...
def verify_cppboost_release_build(
    ...
    for service in ("inventoryservice", "orderservice"):
        # Use the runtime tag selected by the harness, never Docker's
        # implicit :latest or an image from another profile.
        image = (
